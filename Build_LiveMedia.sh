...
# Run the Fedora Remix (livemedia-creator) builder container with Podman.
# Reads Container_Properties from config.yml. Mounts the host output dir on /livemedia-creator
# in the container. Requires a matching OCI image (e.g. ghcr.io/.../fedora-livemedia-builder).
...
if [ ! -f "config.yml" ]; then
    ...
    echo "Please run this script from your FedoraRemix-LiveMedia repository root."
    exit 1
fi
...
# OCI image for lmc/lorax builds (publish your own if needed; not the livecd-creator image)
...
echo -e "${MENU_CYAN}╚═══════════════════════════════════════════════════════════════════╝${MENU_NC}"
echo ""
echo -e "${MENU_WHITE}Finished .iso is usually under:${MENU_NC} ${MENU_YELLOW}$FEDORA_REMIX_LOCATION/result/${MENU_NC} (on host: livemedia-creator/result/ in the mount; name may not match ${SELECTED_KICKSTART}.iso)"
echo ""

CONTAINER_NAME="livemedia-builder"
...
while [ "$i" -lt "$max_s" ]; do
    ...
    if systemctl is-failed --quiet livemedia-builder.service 2>/dev/null; then break; fi
...
# proper labels on bind mounts. Volume suffix :z for shared access on SELinux.
# FEDORA_REMIX_LOCATION is mounted as /livemedia-creator in the container (workdir: …/FedoraRemix)
