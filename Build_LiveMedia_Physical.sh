...
# Native build: update Setup/config.yml, run prepare, then livemedia-creator
# via LiveMedia_Enhanced_Build_Script.sh
#
...
readonly LIVEMEDIA_WORKDIR="/livemedia-creator/FedoraRemix"
readonly PREPARE_BUILD="$SETUP_DIR/Prepare_LiveMedia_Build.py"
readonly PREPARE_WEB="$SETUP_DIR/Prepare_Web_Files.py"
readonly ENHANCED_SCRIPT="$SETUP_DIR/LiveMedia_Enhanced_Build_Script.sh"
...
show_usage() {
    ...
    echo "runs Prepare_LiveMedia_Build.py and Prepare_Web_Files.py, then"
    echo "LiveMedia_Enhanced_Build_Script.sh under ${LIVEMEDIA_WORKDIR}."
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Must be run on the machine where /livemedia-creator will be used; prepare"
...
check_prerequisites() {
    ...
    require_file "$PREPARE_BUILD" "Prepare_LiveMedia_Build.py"
    require_file "$PREPARE_WEB" "Prepare_Web_Files.py"
    require_file "$ENHANCED_SCRIPT" "LiveMedia_Enhanced_Build_Script.sh"

    if [ ! -x "$ENHANCED_SCRIPT" ]; then
        print_message "WARNING" "LiveMedia_Enhanced_Build_Script.sh is not executable; chmod +x"
...
main() {
    ...
    echo ""
    print_message "HEADER" "${ROCKET} Fedora Remix — LiveMedia (native) build"
    echo ""
    ...
    echo -e "${BOLD}${CYAN}║${NC}  ${GREEN}Work directory${NC}    ${WHITE}${LIVEMEDIA_WORKDIR}${NC}                              ${BOLD}${CYAN}║${NC}"
    echo -e "${BOLD}${CYAN}╚══════════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    print_message "INFO" "Running Prepare_LiveMedia_Build.py (sudo python3, cwd: Setup)..."
    (cd "$SETUP_DIR" && sudo python3 ./Prepare_LiveMedia_Build.py)
    ...
    if [ ! -d "$LIVEMEDIA_WORKDIR" ]; then
        print_message "ERROR" "Expected directory missing after prepare: $LIVEMEDIA_WORKDIR"
        exit 1
    fi

    if [ ! -f "$LIVEMEDIA_WORKDIR/LiveMedia_Enhanced_Build_Script.sh" ]; then
        print_message "ERROR" "LiveMedia_Enhanced_Build_Script.sh not found under $LIVEMEDIA_WORKDIR"
        exit 1
    fi

    print_message "INFO" "Starting livemedia-creator in $LIVEMEDIA_WORKDIR (REMIX_KICKSTART=${SELECTED_KICKSTART}, REMIX_INCLUDE_PXEBOOT=${INCLUDE_PXEBOOT})..."
    echo ""
    (cd "$LIVEMEDIA_WORKDIR" && sudo env REMIX_KICKSTART="$SELECTED_KICKSTART" REMIX_INCLUDE_PXEBOOT="$INCLUDE_PXEBOOT" ./LiveMedia_Enhanced_Build_Script.sh)
    local build_rc=$?
